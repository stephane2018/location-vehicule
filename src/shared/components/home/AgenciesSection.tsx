...
function AgencyCard({ agency }: { agency: Agency }) {
  return (
    <div className="group relative flex flex-col rounded-2xl border border-border/50 bg-card overflow-hidden transition-all duration-300 hover:border-primary/20 hover:shadow-lg hover:shadow-primary/5">
      {/* Top gradient strip */}
      <div className={`h-1 bg-linear-to-r ${agency.gradient.replace('/8', '/40').replace('/3', '/20')}`} />

      <div className="flex flex-col gap-4 p-5 flex-1">
        {/* Header row */}
        <div className="flex items-start gap-3">
          <div className={`flex size-10 shrink-0 items-center justify-center rounded-xl bg-linear-to-br ${agency.gradient}`}>
